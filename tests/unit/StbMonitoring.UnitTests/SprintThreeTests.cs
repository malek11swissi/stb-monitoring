...
public sealed class SprintThreeTests
{
 ...
 [Fact]public void Incident_can_be_reopened(){var actor=Guid.NewGuid();var incident=new Incident("INC-2",null,null,null,"Panne","Description",IncidentCategory.Application,IncidentPriority.P3Medium,actor,null,120,480);incident.Assign(actor,actor);incident.Start();incident.Resolve("Corrigé",null);incident.Reopen();Assert.Equal(IncidentStatus.Reopened,incident.Status);Assert.Equal(1,incident.ReopenCount);}
 ...
 [Fact]public void Incident_rejects_invalid_lifecycle_transitions(){var actor=Guid.NewGuid();var incident=new Incident("INC-5",null,null,null,"Panne","Description",IncidentCategory.Application,IncidentPriority.P2High,actor,null,30,120);Assert.Throws<InvalidOperationException>(()=>incident.Pending());Assert.Throws<InvalidOperationException>(()=>incident.Resolve("Corrigé",null));incident.Assign(actor,actor);incident.Start();incident.Resolve("Corrigé",null);Assert.Throws<InvalidOperationException>(()=>incident.Assign(actor,actor));}
}
