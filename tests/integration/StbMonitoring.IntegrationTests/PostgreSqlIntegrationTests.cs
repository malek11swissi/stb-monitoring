...
// Une seule initialisation des migrations pour toute la classe évite que plusieurs
// tests tentent de créer les mêmes tables PostgreSQL au même instant dans Jenkins.
public sealed class PostgreSqlDatabaseFixture : IAsyncLifetime
{
    public string ConnectionString { get; } =
        Environment.GetEnvironmentVariable("STB_TEST_POSTGRES")
        ?? "Host=localhost;Port=55432;Database=stb_monitoring_tests;Username=stb_admin;Password=ChangeMe_Postgres_123!";
...
public sealed class PostgreSqlIntegrationTests(PostgreSqlDatabaseFixture database)
    : IClassFixture<PostgreSqlDatabaseFixture>
{
    ...
    {
        var options=new DbContextOptionsBuilder<MonitoringDbContext>().UseNpgsql(ConnectionString).Options;
        await using var db=new MonitoringDbContext(options);await using var transaction=await db.Database.BeginTransactionAsync();
    ...
    public async Task PostgreSql_persists_incident_comment_history_and_notification()
    {
        var options=new DbContextOptionsBuilder<MonitoringDbContext>().UseNpgsql(ConnectionString).Options;
        await using var db=new MonitoringDbContext(options);await using var transaction=await db.Database.BeginTransactionAsync();
        var suffix=Guid.NewGuid().ToString("N");var user=new User($"integration_{suffix}",$"integration_{suffix}@test.local","hash","Integration","Test");db.Users.Add(user);
        var incident=new Incident($"INC-T-{suffix[..12]}",null,null,null,"Incident intégration","Validation Sprint 3",IncidentCategory.Application,IncidentPriority.P3Medium,user.Id,null,120,480);
    ...
    {
        var options=new DbContextOptionsBuilder<MonitoringDbContext>().UseNpgsql(ConnectionString).Options;
        await using var db=new MonitoringDbContext(options);await using var transaction=await db.Database.BeginTransactionAsync();IOperationsStore store=new OperationsStore(db);
        var suffix=Guid.NewGuid().ToString("N");var user=new User($"detail_{suffix}",$"detail_{suffix}@test.local","hash","Integration","Detail");var incident=new Incident($"INC-D-{suffix[..12]}",null,null,null,"Incident détail","Chargement par identifiant",IncidentCategory.Application,IncidentPriority.P3Medium,user.Id,null,120,480);db.Users.Add(user);db.Incidents.Add(incident);await db.SaveChangesAsync();var existing=incident.Id;
