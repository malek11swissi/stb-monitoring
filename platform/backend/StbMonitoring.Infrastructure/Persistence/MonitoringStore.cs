...
public sealed class MonitoringStore(MonitoringDbContext db):IMonitoringStore
{
 ...
 public async Task<IReadOnlyCollection<CheckResult>>GetResultsAsync(Guid? systemId,Guid? endpointId,int take,CancellationToken ct)=>await db.CheckResults.Include(x=>x.Endpoint).Where(x=>(!systemId.HasValue||x.SystemId==systemId)&&(!endpointId.HasValue||x.EndpointId==endpointId)).OrderByDescending(x=>x.StartedAt).Take(take).ToArrayAsync(ct);
