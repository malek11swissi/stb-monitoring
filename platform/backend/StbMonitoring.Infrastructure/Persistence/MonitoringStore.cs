...
public sealed class MonitoringStore(MonitoringDbContext db):IMonitoringStore
{
 ...
 public Task<MonitoringEndpoint?>GetEndpointAsync(Guid id,CancellationToken ct)=>db.MonitoringEndpoints.Include(x=>x.System).ThenInclude(x=>x.Endpoints).SingleOrDefaultAsync(x=>x.Id==id,ct);
