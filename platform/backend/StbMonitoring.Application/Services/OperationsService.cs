...
public sealed partial class OperationsService(IOperationsStore store,IIncidentNotificationDispatcher incidentNotifications) : IOperationsService
{
    ...
    public async Task EvaluateAsync(CheckResult result, CancellationToken ct)
    {
        ...
        foreach (var rule in rules)
        {
            ...
            if (alert is null)
            {
                ...
                await store.SaveChangesAsync(ct);
                if (rule.NotifyInApp) await NotifyRoles([RoleNames.Supervisor],"ALERT_CREATED", alert.Title, alert.Description, alert.Severity, "Alert", alert.Id, $"/alerts/{alert.Id}", ct);
            }
    ...
    public async Task StartAsync(Guid id, Guid actor, CancellationToken ct) => await Change(id, actor, "INCIDENT_STARTED", x => x.Start(), ct); public async Task PendingAsync(Guid id, Guid actor, CancellationToken ct) => await Change(id, actor, "INCIDENT_PENDING", x => x.Pending(), ct); public async Task ResolveIncidentAsync(Guid id, ResolveIncidentRequest r, Guid actor, CancellationToken ct) { var x = await IncidentRequired(id, ct);if(string.IsNullOrWhiteSpace(r.RootCause))throw new ArgumentException("La cause racine est obligatoire.");if(!await store.HasResolutionProofAsync(id,ct))throw new ArgumentException("Ajoutez au moins une pièce jointe comme preuve avant de résoudre l'incident."); var old = x.Status.ToString(); x.Resolve(r.Summary, r.RootCause, r.CorrectiveAction, r.PreventiveAction, r.ResolutionEvidence); store.AddHistory(new(x.Id, actor, "INCIDENT_RESOLVED", old, x.Status.ToString(), r.Summary));foreach(var userId in await store.GetActiveUserIdsByRolesAsync([RoleNames.Supervisor,RoleNames.ManagerIt],ct))store.AddNotification(new(userId,"INCIDENT_RESOLVED",$"✓ {x.IncidentNumber} résolu",x.Title,AlertSeverity.Info,"Incident",x.Id,$"/incidents/{x.Id}")); await store.SaveChangesAsync(ct); }
    ...
    public async Task CommentAsync(Guid id, AddCommentRequest r, Guid actor, CancellationToken ct) { var incident=await IncidentRequired(id, ct); if(incident.IsArchived)throw new InvalidOperationException("Un incident archivé est en lecture seule."); store.AddComment(new(id, actor, r.Content, r.CommentType, r.IsInternal));var recipients=(await store.GetActiveUserIdsByRolesAsync([RoleNames.Supervisor,RoleNames.ManagerIt],ct)).Concat(incident.AssignedToUserId.HasValue?[incident.AssignedToUserId.Value]:Array.Empty<Guid>()).Where(x=>x!=actor).Distinct();foreach(var userId in recipients)store.AddNotification(new(userId,"INCIDENT_COMMENT",$"Nouveau commentaire · {incident.IncidentNumber}",r.Content,AlertSeverity.Info,"Incident",incident.Id,$"/incidents/{incident.Id}"));await store.SaveChangesAsync(ct); }
    public async Task<IReadOnlyCollection<SlaPolicyResponse>> SlasAsync(CancellationToken ct) => (await store.GetSlasAsync(ct)).Select(Map).ToArray(); public async Task<SlaPolicyResponse> CreateSlaAsync(SlaPolicyRequest r, CancellationToken ct) { var previous=await store.GetSlaForAsync(r.Priority,ct);previous?.SetActive(false);var x = new SlaPolicy(r.Name, r.Description, r.Priority, r.ResponseTimeMinutes, r.ResolutionTimeMinutes, r.WarningPercentage); store.AddSla(x); await store.SaveChangesAsync(ct); return Map(x); }
    ...
    private async Task NotifyRoles(string[] roles,string type, string title, string message, AlertSeverity severity, string entity, Guid id, string url, CancellationToken ct) { foreach (var user in await store.GetActiveUserIdsByRolesAsync(roles,ct)) store.AddNotification(new(user, type, title, message, severity, entity, id, url)); await store.SaveChangesAsync(ct); }
