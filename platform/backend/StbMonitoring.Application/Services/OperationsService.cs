...
public sealed partial class OperationsService(IOperationsStore store,IIncidentNotificationDispatcher incidentNotifications) : IOperationsService
{
    ...
    private async Task Change(Guid id, Guid actor, string action, Action<Incident> change, CancellationToken ct) { var x = await IncidentRequired(id, ct); var old = x.Status.ToString(); change(x); store.AddHistory(new(x.Id, actor, action, old, x.Status.ToString())); await store.SaveChangesAsync(ct); }
