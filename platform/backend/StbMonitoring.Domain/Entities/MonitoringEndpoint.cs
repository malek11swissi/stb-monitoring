...
public sealed class MonitoringEndpoint
{
    ...
    public void Update(string name,string url,CheckType checkType,string httpMethod,int expectedStatusCode,int timeoutSeconds,int intervalSeconds,int degradedThresholdMs,int downThresholdMs,bool isCritical,string? expectedJsonProperty,string? expectedJsonValue){if(string.IsNullOrWhiteSpace(name))throw new ArgumentException("Le nom de l'endpoint est obligatoire.");if(!Uri.TryCreate(url,UriKind.Absolute,out var uri)||uri.Scheme is not("http"or"https"))throw new ArgumentException("URL HTTP/HTTPS invalide.");var method=httpMethod.Trim().ToUpperInvariant();if(method is not("GET"or"HEAD"or"POST"))throw new ArgumentException("Méthode HTTP non supportée.");if(expectedStatusCode is <100 or >599)throw new ArgumentOutOfRangeException(nameof(expectedStatusCode));if(timeoutSeconds is <1 or >300)throw new ArgumentOutOfRangeException(nameof(timeoutSeconds));if(intervalSeconds is <10 or >86400)throw new ArgumentOutOfRangeException(nameof(intervalSeconds));if(degradedThresholdMs<1||downThresholdMs<degradedThresholdMs)throw new ArgumentException("Les seuils de performance sont invalides.");Name=name.Trim();Url=url.Trim();CheckType=checkType;HttpMethod=method;ExpectedStatusCode=expectedStatusCode;TimeoutSeconds=timeoutSeconds;IntervalSeconds=intervalSeconds;DegradedThresholdMs=degradedThresholdMs;DownThresholdMs=downThresholdMs;IsCritical=isCritical;ExpectedJsonProperty=expectedJsonProperty?.Trim();ExpectedJsonValue=expectedJsonValue?.Trim();}
