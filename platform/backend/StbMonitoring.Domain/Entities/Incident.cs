...
public sealed class Incident
{
    ...
    public void Resolve(string summary, string? rootCause) { if (Status is not (IncidentStatus.InProgress or IncidentStatus.Pending or IncidentStatus.Reopened)) throw new InvalidOperationException("L'incident doit être pris en charge avant sa résolution."); if (string.IsNullOrWhiteSpace(summary)) throw new ArgumentException("Le résumé de résolution est obligatoire."); Status = IncidentStatus.Resolved; ResolutionSummary = summary.Trim(); RootCause = rootCause?.Trim(); ResolvedAt = DateTime.UtcNow; SlaStatus = ResolvedAt <= ResolutionDueAt ? SlaStatus.Met : SlaStatus.Breached; UpdatedAt = DateTime.UtcNow; }
