...
export interface EndpointDraft { name:string;url:string;checkType:string;httpMethod:string;expectedStatusCode:number;timeoutSeconds:number;intervalSeconds:number;degradedThresholdMs:number;downThresholdMs:number;isCritical:boolean;expectedJsonProperty:string;expectedJsonValue:string;databaseEngine?:string;databaseHost?:string;databasePort?:number;databaseName?:string;databaseGroup?:string;declaredRole?:string }
