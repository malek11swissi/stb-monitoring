...
<section class="page">
  ...
  @for(user of filtered;track user.id){<tr><td><div class="user-cell"><img [src]="avatar(user)" alt=""><span><strong>{{user.firstName}} {{user.lastName}}</strong><small>&#64;{{user.username}} · {{user.badge||'Membre'}}</small></span></div></td><td>{{user.email}}</td><td><span class="badge">{{user.role}}</span></td><td><span [class]="user.isActive?'status active':'status inactive'">{{user.isActive?'Actif':'Inactif'}}</span></td><td class="actions">@if(auth.hasPermission('users.manage')){<button (click)="edit(user)">Modifier</button><button (click)="active(user)">{{user.isActive?'Désactiver':'Activer'}}</button>}</td></tr>}
  </tbody></table></div>
...
</section>`,styles:[`.user-cell{display:flex;align-items:center;gap:11px}.user-cell img{width:42px;height:42px;object-fit:cover;border-radius:12px;background:#eaf4f6;border:1px solid #d7e5e9}.user-cell span{display:grid}.user-cell small{margin-top:3px;color:#61727e}`]})
